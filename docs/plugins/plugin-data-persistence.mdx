import GithubStarsButton from '../../src/components/HomepageFeatures/buttons/GithubStars';

# Data persistence plugin

<GithubStarsButton user='lyrasearch' repo='plugin-data-persistence' />
...
const filePath = persist(originalInstance, 'binary', './quotes.msp')
```

When bumping the database to disk, the `persist` method will return the path of
the file that was created.

The `persist` method accepts three parameters:
...
const restoredInstance = restore('binary', './quotes.msp')
...
The `restore` method accepts two parameter:
...
const serializedData = persist(originalInstance, 'binary')
```

One difference to notice is that while the `persist` method, when used in a
runtime such as Node.js or Bun, returns the path of the saved file, on the
browser the `persist` method returns the serialized data.
...
const restoredInstance = restore('binary', serializedData)
